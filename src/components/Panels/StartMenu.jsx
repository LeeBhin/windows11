...
export default function StartMenu() {
  ...
  const animStyle = (() => {
    ...
    if (skipExitRef.current && (phase === 'exiting' || phase === 'closed')) {
      if (phase === 'closed') skipExitRef.current = false
      return { transform: TX('calc(100% + 60px)'), transition: 'none', pointerEvents: 'none' }
    }
  ...
  return (
    ...
    >
        ...
        >
          <svg viewBox="0 0 292 300" width="15" height="15" className="flex-shrink-0">
            ...
            <g transform="rotate(45 206 206)">
              <rect x="198" y="207.5" width="118" height="37" rx="18.5" fill="#0071D1" />
            </g>
          </svg>
          <span className="text-[13px] text-gray-400 flex-1 select-none">앱, 설정 및 문서 검색</span>
        </div>
      </div>
      ...
      >
        ...
        <div className="px-7 pb-2">
          <div className="flex items-center justify-between mb-3">
            <span className="text-[14px] font-semibold text-gray-800">고정됨</span>
        ...
        <div className="px-7 py-2">
          <div className="flex items-center justify-between mb-3">
            <span className="text-[14px] font-semibold text-gray-800">맞춤</span>
            <button className="text-[12px] text-gray-500 px-2.5 py-0.5 rounded hover:bg-black/[0.06] transition-colors flex items-center gap-0.5">
              모두 표시
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="9 18 15 12 9 6" /></svg>
            </button>
            ...
            {recommended.map((item) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="text-[12px] text-gray-800 truncate font-medium leading-tight">{item.name}</p>
        ...
        <div className="px-7 py-2 pb-4">
          <div className="flex items-center justify-between mb-4">
            <span className="text-[14px] font-semibold text-gray-800">모두</span>
              ...
              >
                <div className="grid grid-cols-2 gap-2 mb-2.5 justify-items-center">
                  {cat.icons.map((icon, i) => (
                    <span key={i} className="text-[20px] leading-none">{icon}</span>
                  ))}
                </div>
                <p className="text-[11px] text-gray-600 font-medium truncate">{cat.name}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      <div
        ...
        style={{ background: 'rgba(0,0,0,0.025)', borderTop: '1px solid rgba(0,0,0,0.05)' }}
      >
        <button className="flex items-center gap-2.5 px-3 py-1.5 rounded-lg hover:bg-black/[0.06] transition-colors">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="rgba(60,60,60,0.65)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <button className="flex items-center justify-center w-9 h-9 rounded-lg hover:bg-black/[0.06] transition-colors">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgba(60,60,60,0.65)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
